...
class MCPTemplateProcessor {
	...
	private loadSchema() {
		try {
			const schemaPath = resolve(
				__dirname,
				"../mcp-templates/template.schema.json",
			);
	...
	private loadTemplate(templatePath: string): Template {
		...
		if (!isValid) {
			throw new Error(
				`Template validation failed: ${JSON.stringify(this.ajv.errors, null, 2)}`,
			);
		}
	...
	private filterServers(
		...
		if (filter.enabledOnly) {
			filtered = Object.fromEntries(
				Object.entries(filtered).filter(([_, server]) => server.enabled),
			);
		}
	...
	private transformServerName(
		name: string,
		mapping?: Template["transform"]["serverNameMapping"],
	): string {
	...
	private applyConditionalProperties(
		...
		for (const conditional of conditionals) {
			if (this.evaluateCondition(conditional.condition, originalServer)) {
				const processedProperties = this.processTemplateProperties(
	...
	private processTemplateProperties(
		properties: Record<string, any>,
		server: MCPServer,
	): Record<string, any> {
		const result: Record<string, any> = {};
	...
	private executeTemplateFunction(
		functionName: string,
		server: MCPServer,
	): any {
		switch (functionName) {
	...
	public process(
		...
		console.log(
			`Loaded template: ${template.metadata.name} v${template.metadata.version || "1.0.0"}`,
		);

		// Filter servers
		const filteredServers = this.filterServers(
			mcpConfig.servers,
			template.transform.serverFilter,
		);
		console.log(
			`Filtered servers: ${Object.keys(filteredServers).length} of ${Object.keys(mcpConfig.servers).length}`,
		);
		...
		for (const [originalName, server] of Object.entries(filteredServers)) {
			const transformedName = this.transformServerName(
				originalName,
				template.transform.serverNameMapping,
			);

			// Apply property mappings
			let processedServer = this.applyPropertyMappings(
				server,
				template.transform.propertyMappings,
			);

			// Apply static properties
			processedServer = this.applyStaticProperties(
				processedServer,
				template.transform.staticProperties,
			);
	...
	public processAll(
		...
		console.log(
			`Found ${templateFiles.length} template(s): ${templateFiles.join(", ")}`,
		);
