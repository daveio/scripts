...
function normalizeConfig(rawConfig: unknown): MCPConfig {
	...
	if (config.servers) {
		...
		throw new Error(
			"Invalid configuration: missing servers, mcpServers, or extensions section",
		);
	}
	...
	for (const [id, server] of Object.entries(servers)) {
		...
		let command = serverConfig.command as
			| string
			| { path: string; args?: string[]; env?: Record<string, string> };
		...
		normalizedServers[id] = {
			name:
				...
				(serverConfig.display_name as string) ||
				id,
			description: (serverConfig.description as string) || `MCP server: ${id}`,
			command:
				typeof command === "string" ? command : (serverConfig.cmd as string),
			args: args,
			type:
				(serverConfig.type as "stdio" | "builtin" | "sse" | "http") || "stdio",
	...
	try {
		...
		for (const [id, server] of servers) {
			...
			if (server.env && Object.keys(server.env).length > 0) {
				console.log(
					`  ${chalk.gray("Environment:")} ${Object.keys(server.env).join(", ")}`,
				);
			}
	...
	try {
		...
		for (const [id, server] of Object.entries(config.servers)) {
			...
			if (!server.bundled && !server.command) {
				logger.error(
					`Server '${id}' missing required 'command' field (not bundled)`,
				);
				issues++;
			}
		}
		...
		if (issues === 0) {
			...
			logger.error(
				`Configuration validation failed with ${issues} issue${issues === 1 ? "" : "s"}`,
			);
			process.exit(1);
		}
	} catch (error) {
...
async function exportConfig(configPath: string, exportName?: string) {
	...
	try {
		...
		const exports = exportName
			? { [exportName]: config.exports[exportName] }
			: config.exports;
