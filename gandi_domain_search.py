...
res = api.domain.price.list(k, {'tlds': ["{0}.{1}".format(os.environ['GANDI_DOMAIN_NAME'],
                                                          i['name']) for i in tlds], 'action': ['create', 'renew']})
final = []
while True:
    ...
    if num_pending > 0:
        res = api.domain.price.list(
            k, {'tlds': pending, 'action': ['create', 'renew']})
        print("[awaiting {}] ".format(num_pending),
...
concise = [
    [
        x['extension'],
        list(filter(lambda y: y['action']['name'] == 'create', x['prices']))[
            0]['unit_price'][0]['price'],
        list(filter(lambda y: y['action']['name'] == 'renew', x['prices']))[
            0]['unit_price'][0]['price']
